...
#define INDEX_TIMER2_OC 	2  // Timer2 Output Compare Match
#define INDEX_TIMER2_OF 	3  // Timer2 Overflow
#define INDEX_TIMER1_IC 	4  // Timer1 Input capture
#define INDEX_TIMER1_OCA	5  // Timer1 Output Compare match A
#define INDEX_TIMER1_OCB	6  // Timer1 Output Compare match B
#define INDEX_TIMER1_OF		7  // Timer1 Overflow
#define INDEX_TIMER0_OF		8  // Timer0 Overflow
...
void timer2_output_compare_interrupt_enable();
...
void timer2_output_compare_interrupt_disable();
...
void timer2_set_output_compare_interrupt_function(pVoidFunc func);
...
//-------------------------------------------------------------------------------------------------//
	 ...
	 ISR(TIMER2_COMP_vect);					// FOR INTERNAL USE ONLY! 
...
// GICR   : General Interrupt Control Register - Contains external interrupt enable flags (INT0, INT1, INT2)
...
void timer1_output_compareB_interrupt_enable()	{TIMSK |=  1<<OCIE1B;}
void timer1_overflow_interrupt_enable() 		{TIMSK |=  1<<TOIE1;}
void timer2_output_compare_interrupt_enable() 	{TIMSK |=  1<<OCIE2;}
void timer2_overflow_interrupt_enable() 		{TIMSK |=  1<<TOIE2;}
void usart_recieve_interrupt_enable()			{UCSRB |=  1<<RXCIE;}
void usart_transmit_interrupt_enable()			{UCSRB |=  1<<TXCIE;}
void usart_data_reg_empty_interrupt_enable()	{UCSRB |=  1<<UDRIE;}
...
void timer1_output_compareB_interrupt_disable()	{TIMSK &= ~1<<OCIE1B;}
void timer1_overflow_interrupt_disable() 		{TIMSK &= ~1<<TOIE1;}
void timer2_output_compare_interrupt_disable() 	{TIMSK &= ~1<<OCIE2;}
void timer2_overflow_interrupt_disable()		{TIMSK &= ~1<<TOIE2;}
void usart_recieve_interrupt_disable()			{UCSRB &= ~1<<RXCIE;}
void usart_transmit_interrupt_disable()			{UCSRB &= ~1<<TXCIE;}
void usart_data_reg_empty_interrupt_disable()	{UCSRB &= ~1<<UDRIE;}


void int0_set_edge(char edge) {
	switch(edge) {					// Set MCUCR (MCU Control Register)
...
void timer2_set_output_compare_interrupt_function(pVoidFunc func) {
	INTERRUPT_CALLFUNCTION[INDEX_TIMER2_OC] = func; // assign void function to call on interrupt
}
void timer2_set_overflow_interrupt_function(pVoidFunc func) {
...
ISR(TIMER2_COMP_vect) {		INTERRUPT_CALLFUNCTION[INDEX_TIMER2_OC](); }
